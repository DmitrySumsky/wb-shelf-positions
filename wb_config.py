...
for _name, _b in CFG["brands"].items():
    ...
    BRANDS[_name] = {
        ...
        # v2.3.0. Откуда взять строки листа при заводке книги, если образец
        # «Натури пример» бренду не годится: у ORZAX (кабинет ИП1) конкурент —
        # другой ПРОДАВЕЦ того же товара, и состав ведётся в листе «Конкуренты»
        # юнитки кабинета. Читает `shelves_from_competitors.py`.
        "source": dict(_b.get("source") or {}),
    }
